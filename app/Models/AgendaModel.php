...
class AgendaModel extends Model
{
    ...
    public function showAgenda($idMatkul)
    {
        ...
        $query = $table->select('agenda.*,matkul.*,agenda.id as id')->join('matkul', 'agenda.id_matkul=matkul.id')->join('users', 'matkul.id_user=users.id')->whereIn('agenda.id_matkul', $idMatkul)->where('users.deleted_at', null)->where('matkul.deleted_at', null)->where('agenda.deleted_at', null)->where('matkul.deleted_at', null);
